...
public class OptionalPostcodeValidatorTest {
    private static final String VALID_OUTWARD_CODE = "A0";
    private static final String VALID_INWARD_CODE = "0AA";

    private static Validator validator;

    private final EmployersAddressQuestionWrapper employersQuestionWrapper = new EmployersAddressQuestionWrapper();
    private final ProvidersAddressQuestionWrapper providersAddressQuestionWrapper = new ProvidersAddressQuestionWrapper();
    ...
    @Test
    public void a_postcode_will_validate_when_we_have_an_empty_input() {
        assertTrue(employersQuestionWrapper.validate(""));
        assertTrue(providersAddressQuestionWrapper.validate(""));
    }

    @Test
    public void a_postcode_will_validate_when_we_have_a_simple_example() {
        assertTrue(employersQuestionWrapper.validate(VALID_OUTWARD_CODE + " " + VALID_INWARD_CODE));
        assertTrue(providersAddressQuestionWrapper.validate(VALID_OUTWARD_CODE + " " + VALID_INWARD_CODE));
    }
    ...
    public void a_postcode_outward_code_will_validate_when_we_match_a_character_sequence_of_A0_or_A00_or_AA0_or_A0A_or_AAA_or_AA00_or_AA0A_where_A_is_any_letter_and_0_is_any_numchar(
            String postcode) {
        assertTrue(employersQuestionWrapper.validate(postcode));
    }
    ...
    public void a_postcode_outward_code_will_not_validate_when_it_doesnt_match_a_character_sequence_of_A0_or_A00_or_AA0_or_A0A_or_AAA_or_AA00_or_AA0A_where_A_is_any_letter_and_0_is_any_numchar(
            String postcode) {
        assertFalse(employersQuestionWrapper.validate(postcode));
        assertFalse(providersAddressQuestionWrapper.validate(postcode));
    }
    ...
    private static class EmployersAddressQuestionWrapper {
        boolean validate(String postcode) {
            ...
            Set<ConstraintViolation<EmployersAddressQuestion>> violations = validator.validate(addressQuestion);
            return violations.isEmpty();
        }
    }

    private static class ProvidersAddressQuestionWrapper {
        boolean validate(String postcode) {
            ProvidersAddressQuestion addressQuestion = new ProvidersAddressQuestion();
            ...
            Set<ConstraintViolation<ProvidersAddressQuestion>> violations = validator.validate(addressQuestion);
            return violations.isEmpty();
        }
    }
}
